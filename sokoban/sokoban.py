...
def start_game():
    ...
    if level > 0:
        ...
        sys.exit(2)       
def press(move):
    if move == 'L':  pyautogui.press('left', interval = 0.01) 
    if move == 'U':  pyautogui.press('up' ,interval = 0.01) 
    if move == 'R':  pyautogui.press('right', interval = 0.01) 
    if move == 'D':  pyautogui.press('down' ,interval = 0.01) 
...
def Calculate():
    ...
    imp = pygame.image.load('images/loading1.jpg').convert()    
    scrn.blit(imp, (0,0))    
    pygame.display.flip()
    status = True
    while (status):   
        global moves    
        moves = run(method,'level'+level+'.txt')
        if moves:
            status = False
def Start_Solve():
    complete= False
...
def select_Level():
    ...
    while True:
        ...
        PLAY_BACK = Button(image=None, pos=(740, 600), 
                    text_input="BACK", font=get_font(40), base_color="White", hovering_color="Green")
        
        PLAY_BACK.changeColor(PLAY_MOUSE_POS)
        PLAY_BACK.update(SCREEN)

        PLAY_START = Button(image=None, pos=(740, 400), 
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if PLAY_START.checkForInput(PLAY_MOUSE_POS):
                    game = Game('sokobanLevels/level'+ str(int(level)) + '.txt',int(level))
                    method=''
                    moves = ''
                    play()
            if event.type == pygame.KEYDOWN:
...
def Solve_choice():
    while True:
        ...
        OPTIONS_RECT = OPTIONS_TEXT.get_rect(center=(x-700,y-800))
        SCREEN.blit(OPTIONS_TEXT, OPTIONS_RECT)

        SOLVE_DFS = Button(image = None, pos = (640, 200),
                           text_input="DFS", font =get_font(40), base_color="Black", hovering_color="White")
        SOLVE_BFS = Button(image = None, pos = (640, 300),
                           text_input="BFS", font =get_font(40), base_color="Black", hovering_color="White")
        SOLVE_ASTAR = Button(image = None, pos = (640, 400),
                           text_input="ASTAR", font =get_font(40), base_color="Black", hovering_color="White")
        SOLVE_UCS = Button(image = None, pos = (640, 500),
                           text_input="UCS", font =get_font(40), base_color="Black", hovering_color="White")
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                if OPTIONS_BACK.checkForInput(OPTIONS_MOUSE_POS):
                    play()
                if SOLVE_DFS.checkForInput(OPTIONS_MOUSE_POS):
                    method ='dfs'
                    # print(method)
                if SOLVE_ASTAR.checkForInput(OPTIONS_MOUSE_POS):
                    method ='astar'
                    # print(method)
                if SOLVE_UCS.checkForInput(OPTIONS_MOUSE_POS):
                    method ='ucs'
                    # print(method)
                if SOLVE_BFS.checkForInput(OPTIONS_MOUSE_POS):
                    method ='bfs'
                    # print(method)
...
def main_menu():
    ...
    while True:
        ...
        QUIT_BUTTON = Button(image=pygame.image.load("assets/Quit Rect.png"), pos=(740, 550), 
