...
def PosOfGoals(gameState):
    ...
    return tuple(tuple(x) for x in np.argwhere((gameState == 4) 
                                               | (gameState == 5))) 
...
def run(method,options):
    ...
    global gameState, posGoals, posWalls
    with open('sokobanLevels/'+options,"r") as f: 
       layout = f.readlines()[1:]
